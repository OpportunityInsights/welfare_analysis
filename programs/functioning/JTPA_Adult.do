...
* Provide job training for economically disadvantaged adults.
...
local tax_rate_assumption = "$tax_rate_assumption" //takes value paper internal,
	// mixed, or cont. If "cont", also need to specify a number in tax_rate_cont
	*Mixed tax rate: "Observed" values used for 2 years. Projected thereafter.
local tax_rate_cont = $tax_rate_cont
local payroll_assumption = "no"
local afdc_assumption = "$afdc_assumption" // Yes = AFDC reductions continue in
	//subsequent years, No = lump sum one-time reduction
...
local earn_impact_obs_m = 1599 // Bloom Table 8, JTPA Adult Row 18 (sums earnings gain and wage subsidy; increase in earnings observed over 30m). Includes wage subsidy.
local earn_impact_obs_m_p =  runiform(0.05,0.1) //"between .05 and .1" in Bloom Table 3
local earn_impact_obs_f = 1837 // Bloom Table 8, JTPA Adult Row 18 (increase in earnings observed over 30m). Includes wage subsidy.
local earn_impact_obs_f_p =  runiform(0,0.01) //"between 0 and .01" in Bloom Table 3

local static_welfare_fe_m = -334 // Bloom Table 8, JTPA Adult Row 13 (reduction in welfare payment from being in program during 30m)
...
local static_welfare_fe_f = 235 // Bloom Table 8, JTPA Adult Row 13 (reduction in welfare payment from being in program during 30m)
...
****************************************************
local samp_size_m = 5102 //Bloom Table 3
local samp_size_f = 6102 // Bloom Table 3

local year_jtpa = 1988 //random assignment lasted about 15 months in each SDA, on average,
	// beginning in November 1987 and ending in September 1989

*Mean control group earnings (over 30-month period):
local control_group_mean_m = 18496 // Bloom et al p 559
local control_group_mean_f = 12241 // Bloom et al p 559

*Reduction in private spending:
local reduc_private_spending_m = 100 // Bloom Table 8
local reduc_private_spending_f = 56 // Bloom Table 8
...
local age_stat_m = ((22+29)/2)*0.455 + ((30+44)/2)*0.425 + ((45+54)/2)* 0.08 + 55*0.04
//From: https://www.upjohn.org/sites/default/files/2019-02/njtpareport_0.pdf exhibit 7.9
local age_stat_f = ((22+29)/2)*0.440 + ((30+44)/2)*0.440 + ((45+54)/2)* 0.074 + 55*0.046
//From: https://www.upjohn.org/sites/default/files/2019-02/njtpareport_0.pdf exhibit 7.8

local program_cost_m = 1175 // Bloom Table 8, JTPA Adult Row 2 (sums training costs and wage subsidy)
local program_cost_f = 1381 // Bloom Table 8, JTPA Adult Row 2 (sums training costs and wage subsidy)

local wage_sub_expend_m = 244 // Bloom Table 8 (wage subsidy as part of training program for the 30m)
local wage_sub_expend_f = 154 // Bloom Table 8 (wage subsidy as part of program for the 30m)
...
foreach s in f m {
	...
	*Compute after-tax earnings and transfer impacts:
	...
	local FE_`s' = (`obs_earn_impact_`s''+`wage_sub_expend_`s'')*`fe_rate_obs_`s'' ///
					+ `proj_earn_impact_`s''*`fe_rate_proj_`s'' + `afdc_supp_`s''
}
