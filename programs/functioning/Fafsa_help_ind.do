********************************************
/* 0. Program: FAFSA Help - Indendent */
********************************************
...
Results from the H&R Block FAFSA experiment." The Quarterly Journal of Economics 127, no. 3 (2012): 1205-1242.*/
...
/*
*Independent Year Impact
local year_imp_ind = 0.027 //Bettinger et al.  2012, Table 6
local year_imp_ind_se = 0.016

*Independent Pell Impact 
local year_imp_ind_pell = 0.47 //Bettinger et al.  2012, Table 7
local year_imp_ind_pell_se = 0.018

*Independent Enrollment Impact
local enroll_imp_ind = 0.015 //Bettinger et al.  2012, Table 3
local enroll_imp_ind_se = 0.007

*Independent Enrollment Impact + Pell 
local enroll_imp_ind_pell = 0.030 //Bettinger et al.  2012, Table 3
local enroll_imp_ind_pell_se = 0.007
*/
...
local proj_start_age = 26 //Note: Bettinger et al. Table 2 says the average age is 25.911
local proj_short_end = 32
local impact_age = 34
local impact_age_neg = round((26+32)/2)
local project_year = 2008 
local proj_age_end = 32

*Assumptions of Age for Earnings Gain Projection
local proj_start_age_pos = 33
local impact_age_pos = 34
local project_year_pos = 2015 

*Independent Cost Impact
local cost_imp_ind = 100 //Bettinger et al.  2012, Page 1238
local cost_tot_ind = 4157
/*
...
a Pell Grant is determined by the .030 treatment effect found in Table 3. 
...
*Household Income 
local hh_income = 16404 //Bettinger et al. 2012, Table 2
/*
...
local attend_4_year = 0.005 //Bettinger et al. 2012, Table 4
local attend_2_year = 0.004 //Bettinger et al. 2012, Table 4
local attend_pub = 0.011 //Bettinger et al. 2012, Table 4
local attend_priv = 0.001 //Bettinger et al. 2012, Table 4
...
local years_impact = `year_imp_ind'
...
local years_impact = `enroll_imp_ind'*`years_enroll_bach'*`frac_4_year' + ///
	`enroll_imp_ind'*`years_enroll_cc'*(1-`frac_4_year')
...
local person_contribution = `pell_2008' - `cost_tot_ind'
/*
...
if "`proj_type'" == "growth forecast" {
	...
	*Initial Earnings Decline 
	est_life_impact `pct_earn_impact_neg', ///
		...
		income_info(`hh_income') income_info_type(counterfactual_income) ///
...
local induced_fraction = (`year_imp_ind'/`year_imp_ind_pell')
/*
Note: We assume that the fraction of induced individuals is determined by the number of new 
years of college enrollment as a fraction of the total number of new individuals receiving Pell Grants.
*/
...
/*
Note: This corrects for the case where a correlation between the treatment effects for pell recipients and
new enrollees set at 1. In that case we might find that the number of new years of schooling exceeds the 
number of total new years of Pell receipt. Given that the nature of the FAFSA form in securing student aid, 
we rule out the unlikely occurence that the policy induces individuals to enroll in school without
securing aid. 
*/
...
if "`outcome_type'" == "enrollment"{
local induced_fraction = (`enroll_imp_ind'/`enroll_imp_ind_pell')
/*
...
/*
Note: This corrects for the case where a correlation between the treatment effects for pell recipients and
new enrollees isn't implemented. In that case we might find that the number of new enrollees exceeds the 
number of total new pell recipients. Given that the nature of the FAFSA form in securing student aid, 
we rule out the unlikely occurence that the policy induces individuals to enroll in school without
securing aid. 
*/
...
if "`outcome_type'" == "enrollment"{
	local years_impact_disc = `enroll_imp_ind'*`years_enroll_bach_disc'*`frac_4_year' + ///
		`enroll_imp_ind'*`years_enroll_cc_disc'*(1-`frac_4_year')
}

*Calculate Cost of Pell Grants Amongst Those Who Would Have Already Enrolled
local pell_cost_non_induced = `cost_imp_ind'*(1/`enroll_imp_ind_pell')*`years_impact_disc'*((`enroll_imp_ind_pell' - `enroll_imp_ind')/`enroll_imp_ind_pell')
/*
...
The Pell costs for those who would have already enrolled is equal to the cost of a year for a new enrollee "`cost_imp_ind'*(1/`enroll_imp_ind_pell')"
...
((`enroll_imp_ind' - `enroll_imp_ind_pell')/`enroll_imp_ind_pell'). 
*/


if "${got_Fafsa_help_ind_OH_costs}"!="yes" {
	cost_of_college, year(2008) state("OH") type_of_uni("any")
	global cost_of_college_2008_oh_any_ind = `r(cost_of_college)'
	
	global got_Fafsa_help_ind_OH_costs yes
	}
local cost_of_college_2008_oh_any = $cost_of_college_2008_oh_any_ind

if "${got_Fafsa_help_ind_NC_costs}"!="yes" {	
	cost_of_college, year(2008) state("NC") type_of_uni("any")
	global cost_of_college_2008_nc_any_ind = `r(cost_of_college)'
	
	global got_Fafsa_help_ind_NC_costs yes
	}
local cost_of_college_2008_nc_any = $cost_of_college_2008_nc_any_ind

if "`private_costs_gov'"=="no" {
	local induced_cost_private = `years_impact_disc'*`cost_imp_ind'*(1/`enroll_imp_ind_pell')*(1-`frac_public')
}
...
were at two-year or four-year schools. An alterante method would split the sample by the fraction of two-year attendees and 
...
local total_cost = `base_program_cost' - `FE' + `enroll_cost'

*Calculate program costs (Administrative costs and Pell costs without additional costs crowded in 

local pell_cost_induced = `cost_imp_ind'*`years_impact_disc'*(1/`enroll_imp_ind_pell')
...
if "`wtp_valuation'" == "cost"{
	local wtp_not_induced = `participation_cost' +`hrblock_cost' + `pell_cost_non_induced'
...
if "`wtp_valuation'" == "nudge"{
	// no clear lower bound on valuation, choose valuation at participation cost	
