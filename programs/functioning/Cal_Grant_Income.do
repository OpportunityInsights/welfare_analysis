...
/*
**Estimated effects at Income threshold**

*Estimated effect on log labor income
local labor_income_log_change_inc = -0.020 //Bettinger et al. 2019 Table 3
local labor_income_log_change_inc_se = 0.024 //Bettinger et al. 2019 Table 3

*Estimated effect on AGI adjusted for household size
local agi_log_change_inc = -0.005 //Bettinger et al. 2019 Table 3
local agi_log_change_inc_se = 0.022 //Bettinger et al. 2019 Table 3

*Cost of Grant
local cost_grant_inc = 8115.30 //Bettinger et al. Table 2
local cost_grant_inc_se = 286.167 //Bettinger et al. Table 2

*Graduation Effects
local bach_deg_change = 0.030 //Bettinger et al. Table 2
local bach_deg_change_se = 0.019 //Bettinger et al. Table 2

*Attendance Effects
local enroll_cc_change = -0.029 //Bettinger et al. Table 2
local enroll_cc_change_se = 0.013 //Bettinger et al. Table 2
local enroll_pub_change = -0.046 //Bettinger et al. Table 2
local enroll_pub_change_se = 0.014 //Bettinger et al. Table 2
local enroll_priv_change = 0.056 //Bettinger et al. Table 2
local enroll_priv_change_se = 0.012 //Bettinger et al. Table 2

*Instructional Costs
local inst_cost = -103.078 // Bettinger et al. Table 4
local inst_cost_se = 187.745 // Bettinger et al. Table 4
*/
...
local mean_earning_inc = 46978 //Bettinger et al. 2019 Table 1
*Mean Earnings in Sample after 10-14 years -- AGI
local mean_earning_inc_agi = 57346 //Bettinger et al. 2019 Table 1
...
local year_15_age = round(18.24+15) //Table 1 has mean age at application of 18.24. 15 years after application.
...
if "`income_measure'" == "log income"{ //earnings based on log income
	local earn_prc = exp(`labor_income_log_change_inc') - 1 // convert to % earnings impact
	local mean_earning_comb = `mean_earning_inc' // combined treatement + control
...
if "`income_measure'" == "agi"{ //earnings based on household adjusted AGI
	local earn_prc = exp(`agi_log_change_inc') - 1 // convert to % earnings impact
	local mean_earning_comb = `mean_earning_inc_agi' // combined treatement + control
	local mean_earnings = `mean_earning_comb'*(1-(`earn_prc'/2)) // assume 50% control to get control earnings
}
...
local enroll_cost = `bach_deg_change'*`years_bach_deg_disc'*`cost_of_college_fte_ca_2000_cc' + ///
...
local program_cost_unscaled = `cost_grant_inc'
local program_cost = `cost_grant_inc'*(1 - `bach_deg_change' - `enroll_pub_change' - `enroll_priv_change')
di `cost_grant_inc'
di `program_cost'
...
local enrollment_FE = `enroll_cost'-(`cost_grant_inc'*`frac_induced')
...
if "`wtp_valuation'" == "post tax" {
	*Those induced value at post tax earnings impacts, those not value at transfer
	...
	local WTP_non_induced = `frac_non_induced'*`cost_grant_inc'
	local WTP = `WTP_induced' + `WTP_non_induced'
}

if "`wtp_valuation'" == "cost" {
	*Non-induced value at transfer 1:1
	local WTP_non_induced = `cost_grant_inc'*`frac_non_induced'
	*Induced value at potentially lower fraction `val_given_marginal'
	local WTP_induced = `val_given_marginal'*`cost_grant_inc'*`frac_induced'
	*Combine
...
di `cost_grant_inc'*`frac_non_induced' // Mechanical Cost
di `cost_grant_inc'*`frac_induced' // Behavioral Cost Program
...
global cal_inc_grant = `cost_grant_inc'
global cal_inc_ed_exp = `enrollment_FE'
global cal_inc_14y_tax = `total_earn_14y'*`tax_rate_cont'
global cal_inc_life_tax = -1*`tax_FE' - ${cal_inc_14y_tax}



* income globals
deflate_to 2015, from(`usd_year')
global inc_stat_`1' = `mean_earning_inc' * r(deflator)
global inc_type_stat_`1' = "individual"
global inc_year_stat_`1' = `=`projection_year'-1'
global inc_age_stat_`1' = `=`year_15_age'-1'

global inc_benef_`1' = `mean_earning_inc' * r(deflator)
