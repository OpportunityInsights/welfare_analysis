**********************************************************************
/* 0. Program: National Tuition Cuts  */
**********************************************************************
...
/*
	local effect_t_0 = -0.016 //Deming and Walters (2017) Table A6
	local effect_t_0_se = .072 //Deming and Walters (2017) Table A6

	local effect_t_1 = .066 //Deming and Walters (2017) Table A6
	local effect_t_1_se = .100 //Deming and Walters (2017) Table A6

	local effect_t_2 = .031 //Deming and Walters (2017) Table A6
	local effect_t_2_se = .118 //Deming and Walters (2017) Table A6

	local effect_t_3 = -0.073 //Deming and Walters (2017) Table A6
	local effect_t_3_se = .123 //Deming and Walters (2017) Table A6
...
local enroll_change = (`treatment_tuition'/`tuition_avg')*`sum_effects_adj'
// Note that this % increase in enrollees due to a 1000 change in the tuition.
...
/* 5. Cost Calculations */
**************************
local program_cost_unscaled = `treatment_tuition' + (`treatment_tuition' * `enroll_change')
local program_cost = `treatment_tuition'
local enroll_change_disc = (`treatment_tuition'/`tuition_avg')*`sum_effects_adj_disc'


// alternative specification -- use costs/tuition cited in paper
if "`delta_costs'" == "no" {
	...
	*Calculate Cost of Additional enrollment
	if "${got_Nat_tuition_costs}"!="yes" {
		cost_of_college, year(2001) state(`any') name(`any') type_of_uni("any")
		global Nat_tuition_cost_of_coll = r(cost_of_college)
		global Nat_tuition_tuition_coll = r(tuition)

	}

	local spend_coll = $Nat_tuition_cost_of_coll
	local tuition_coll = $Nat_tuition_tuition_coll
...
local enroll_cost = `cost_college_change' - `enroll_change_disc'*`treatment_tuition'- `priv_cont'
...
if "`wtp_valuation'" == "post tax" {
	...
	*Uninduced value at program cost
	local wtp_not_induced = `treatment_tuition'
	*Sum
...
if "`wtp_valuation'" == "cost" {
	*Induced value at fraction of transfer: `val_given_marginal'
	local wtp_induced = `treatment_tuition' * `enroll_change'*`val_given_marginal'
...
di `treatment_tuition' // Mechanical Cost
di (`treatment_tuition' * `enroll_change') // Behavioral Cost Program
...
di `enroll_cost'
di `sum_effects_adj'
di `sum_effects_adj_disc'
di `cost_college_change'
di `enroll_change'
di `enroll_change_disc'
di `increase_taxes'
di `total_cost'
di `MVPF'
